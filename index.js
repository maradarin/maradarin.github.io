...
app.post('/inreg',function(req,res) {
  var dateForm=new formidable.IncomingForm()
	dateForm.parse(req, function(err, fields, files){
      ...
      dataInreg: new Date(),
      rol: "user",
      artisti: fields.artisti,
      gr_rad: fields.gr_rad
    }
...
app.post('/login',function(req,res) {
  var dateForm=new formidable.IncomingForm()
    ...
    else { console.log("Nu exista utilizatorul") }
	})
})
...
app.get('/*', function(req, res) {
		var usrn=req.session ? (req.session.utilizator? req.session.utilizator.username : null) : null;
	res.render('html/'+req.url.replace(/\./g, ""), {username: usrn}, function(err, rezultatRender){
		if (err) {
...
/*va tratarea erorii 404 se pune la final (aici o sa fie un caz general, pe care intra orice cerere). Daca s-a gasit mai sus, se opreste acolo; nu mai ajunge aici.*/
